...
The image data collected by an unmanned aerial vehicle or drone.
...
%XXX, or some vertices are added or deleted.
...
\centering
\includegraphics[width=0.8\linewidth]{pix/Figures/Compare1.eps}
\caption{The viewing distribution of a hot message originated from Beijing.}
\label{fig:ChinaMap}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.3\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{pix/Figures/Compare2.eps}
\caption{The change of modularity towards Information Increment.}
\label{fig:stability}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{pix/Figures/Compare3.eps}
\caption{Performance of UDM on WM and Gowalla datasets.}
\label{fig:UDM}
\end{minipage}
\begin{minipage}[t]{0.3\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{pix/Figures/Compare4.eps}
\caption{Performance of UDM on WM and Gowalla datasets.}
\label{fig:UDM}
\end{minipage}
\end{figure*}
...
Figure show the time consuming in grid of different algorithms on different granularity of measurement time. We observe that our algorithm uses less time when consider flight time only. The difference between our algorithm and sequential algorithm decreases as measurement time increases and when measurement time is 5 seconds total time of two algorithms are equal.
...
\subsection{Results of different loads}
Figure show the battery coverage in grid with different loads. As load increases, coverage set of three algorithms decreases and the difference among three algorithms decrease as well.

\subsection{Results of simulations}
We simulate grids of different length and width in this subsection.

Figure show the flight time in grid with different width and length of different algorithms. We observe that our algorithm uses less time in every grid. And fixing width (or length), difference among algorithms increases as length (or width) increases.

Similarly, we have the same result when considering battery coverage. When fixing width (or length), our algorithm could cover more area than other algorithms.
