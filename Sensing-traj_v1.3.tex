...
A dominating path $L$ for graph $G=(V,E)$ is path $L \subset V$ such that every vertex not in $L$ has a neighbor on $L$ which is a special connected dominating set. In a dominating path $L$,
\begin{itemize}
  \item there exists a dominating set $D_L$ of $G$, and vertices in $D_L$ are called \emph{dominating vertices} of path $L$;
  \item and the other vertices in $L \setminus D_l$ are called \emph{connecting vertices} of path $L$.
\end{itemize}
\end{defn}
% Dominating path:  A connecting vertex $v$ is a vertex whose coverage set is covered by other vertices while a dominating vertex is on the contrary.
...
\subsubsection{Decomposing the minimum dominating path in $L_{m,n+3}$ }
In the following lemmas and theorems, we derive the lower bound for the number of vertices needed to add into the minimum dominating path of $L_{m,n}$ so as to find the minimum dominating path in the expanded grid $L_{m,{n+3}}$.
...
Let $L^*$ denote minimum dominating path of $L_{m,n}$, and $L$ denote a minimum dominating path of $L_{m,(n+3)}$. Then there exists at least one minimum dominating path $L$ in $L_{m,(n+3)}$, such that the number of dominating vertices of path $L$ is no less than the number of dominating vertices of path $L^*$. That is, $|D^c_{1,n}(L)| \geq |D(L^*)|$.
...
Given $L$ as the minimum dominating path of the grid $L_{m,n+3}$, let $D^c_{n+1,n+3}(L)$ denote the set of dominating vertices of path $L$ from the $n+1$-th column to the $n+3$-th column of grid $L_{m,n+3}$; and let $C^{ccon}_n(L)$ denote the set of connecting vertices of path $L$ between the $n$-th column and the $n+1$-th column. Then, we have the following lemma.
\begin{lem}~\label{lem:3dominating}
The number of dominating vertices of path $L$ from the $n+1$-th column to the $n+3$-th column of grid $L_{m,n+3}$, plus the number of connecting vertices of path $L$ between the $n$-th column and the $n+1$-th column is no less than $m$. That is $|D^c_{n+1,n+3}(L)| + |C^{ccon}_n(L)| \geq m$. Further, if $3 \nmid m$, then $|D^c_{n+1,n+3}(L)| + |C^{ccon}_n(L)| \geq m+1$.
\end{lem}
...
\subsubsection{How many vertices are needed for finding the minimum dominating path in $L_{m,n+3}$ }
...
Since the left part is a grid $L_{m,n}$, the following theorem tells the relationship between the minimum dominating paths of $L_{m,n+3}$ and $L_{m,n}$.
\begin{thm}~\label{thm:3period}
The minimum dominating paths of $L_{m,n+3}$ contains at least $m$ more vertices than that of $L_{m,n}$. That is $\gamma_l(G) \geq \gamma_l(G^*)+m$. Further, when $3 \nmid m$, $\gamma_l(G) \geq \gamma_l(G^*)+m+1$.
\end{thm}
...
\centering
\includegraphics[height=0.7\linewidth]{pix/sensing-traj/meta_struct1.pdf}
\caption{(a)}
\label{fig:meta_struct1_1}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[height=0.7\linewidth]{pix/sensing-traj/meta_struct2.pdf}
\caption{(b) 1}
\label{fig:meta_struct1_2}
\end{minipage}
\hfill
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[height=0.7\linewidth]{pix/sensing-traj/meta_struct3.pdf}
\caption{(b)}
\label{fig:meta_struct1_3}
\end{minipage}
\caption{Different cases of meta struct 1. Red vertices are extra vertices in minimum dominating path of $L_{m,n+3}$, blue vertices are shifted connecting vertices, Black vertices are vertices in minimum dominating path of $L_{m,n}$. The dashed line separates $G^c_{1,n}$ and $G^c_{n+1,n+3}$.}
\end{figure*}

Although we have proof the lower bound for minimum dominating path in $L_{m,n}$, in many cases we can not reach it. Therefore, we consider structures of minimum dominating path instead and give \textbf{meta structs} to construct them.

\textbf{Struct~1: $\gamma_l(L_{m,n+3})-\gamma_l(L_{m,n})=m$}.
There are two cases for meta struct 1.

\begin{enumerate}
  \item When $m \equiv 0 \pmod 3$, we could select vertexes in $G^r_2$, $G^r_5$..., $G^r_m-1$ in $G^c_{n+1,n+3}$ into minimum dominating path with connecting vertices shift from $G^c_n$ to $G^c_{n+3}$, as shown in Figure~\ref{fig:meta_struct1_1}. And this structure is scalable.
...
\textbf{Struct~2: $\gamma_l(L_{m,n+3})-\gamma_l(L_{m,n})=m+1$}.
\begin{figure*}
\begin{minipage}[t]{0.13\textwidth}
\centering
\includegraphics[height=1\linewidth]{pix/sensing-traj/meta_struct4.pdf}
\caption{(a)}
\label{fig:meta_struct2_1}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.13\textwidth}
\centering
\includegraphics[height=0.8\linewidth]{pix/sensing-traj/meta_struct5.pdf}
\caption{(b)}
\label{fig:meta_struct2_2}
\end{minipage}
\hfill
\begin{minipage}[t]{0.13\textwidth}
\centering
\includegraphics[height=0.8\linewidth]{pix/sensing-traj/meta_struct6.pdf}
\caption{(c)}
\label{fig:meta_struct2_3}
\end{minipage}
\begin{minipage}[t]{0.25\textwidth}
\centering
\includegraphics[height=0.4\linewidth]{pix/sensing-traj/meta_struct7.pdf}
\caption{(d)}
\label{fig:meta_struct2_4}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.2\textwidth}
\centering
\includegraphics[height=0.5\linewidth]{pix/sensing-traj/meta_struct8.pdf}
\caption{(e)}
\label{fig:meta_struct2_5}
\end{minipage}
\hfill
\caption{Different cases of meta struct 2.}
\end{figure*}

There are five cases for meta struct 2.

\begin{enumerate}
  \item When $m \equiv 2 \pmod 3$, we could construct minimum dominating path using similar struct like Figure~\ref{fig:meta_struct2_1} as shown in Figure~\ref{} which is also scalable.
  \item When $v_{1,n}$ belong to minimum dominating path in $L_{m,n}$, we could construct as Figure~\ref{fig:meta_struct2_2}.
  \item When $G^c_n \backslash v_{1,n}$ belong to minimum dominating path in $L_{m,n}$, we could construct as Figure ~\ref{fig:meta_struct2_3}.
  \item When $n \equiv 0 \pmod 3$, we could rotate direction of minimum dominating path of $L_{m,n}$ and construct with meta struct 1. If rotation only bring one extra vertex, we could have construct as Figure~\ref{fig:meta_struct2_4} and $\gamma_l(L_{m,n+3})-\gamma_l(L_{m,n})=m+1$.
  \item When $n \equiv 2 \pmod 3$ and $n \geq 8$, we could use structure in Figure~\ref{fig:meta_struct2_5} once to construct minimum dominating path in $L_{m,n+3}$.
\end{enumerate}

\textbf{Struct~3: $\gamma_l(L_{m,n+3})-\gamma_l(L_{m,n})=m+2$}.
\begin{figure}[htbp]
 \centering
 \includegraphics[height=0.3\linewidth]{pix/sensing-traj/meta_struct9.pdf}
\caption{Typical case of meta struct 3.}
\label{fig:meta_struct3}
\end{figure}

Since there are several cases for meta struct 3. We consider the most common and useful case.

When there are no vertices in $G^c_n$ belong to minimum dominating path $L$ in $L_{m,n}$, vertices in $G^c_{n-1}$ must belong to $L$. Then, we could construct minimum dominating path in $L_{m,n+3}$ with structure shown in Figure~\ref{fig:meta_struct3}.
...
\begin{figure}[htbp]
 \centering \includegraphics[width=2.4in]{pix/sensing-traj/path_1.pdf} %\vspace{-0.1in}
\caption{Minimum dominating path for $L_{1,n}$, $L_{2,n}$ and $L_{3,n}$ }
\label{fig:path_1}
\end{figure}

Since connected dominating set is also dominating path when $m<4$ and $m<n$, we have the same structure as shown in Figure~\ref{fig:path_1} of \cite{Hamburger2007Routing}.
Therefore, we have
\begin{enumerate}
...
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_2.pdf}
...
\begin{minipage}[c]{0.45\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_3.pdf}
...
Since minimum dominating path in $L{m,1}$ contains $m-2-\lceil \frac{m}{3} \rceil$ connecting vertices, when $m \equiv 0 \pmod 3$, $n \equiv 1 \pmod 3$, we could use meta struct 1 to construct $L_{m,n}$. we have the following structure as shown in Figure~\ref{fig:path2_1}.

Therefore, when $m \equiv 0 \pmod 3$, $n \equiv 1 \pmod 3$, we have $\gamma_l(L_{m,n})=3ab+3a-2$.

Similarly, when $m \equiv 0 \pmod 3$, $n \equiv 1 \pmod 3$, we use meta struct 1 when $n=1$, then we use meta struct 2 as $n$ increases. We have the following structure as shown in Figure~\ref{fig:path2_2}.
...
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_4.pdf}
...
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_5.pdf}
...
\begin{minipage}[t]{0.32\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_6.pdf}
...
We could use reduction on the structure to construct minimum dominating path of $L_{m,n}$ in this case.

When $m \equiv 0 \pmod 3$, $n \equiv 0 \pmod 3$, if $m=6$, we could construct $L_{6,n}$ using meta struct 1. Assume the result hold for $m=k$, then we could get minimum dominating path for $L_{k+3,k}$ from $L_{k,k+3}$, since none of meta struct 1 and meta struct 2 fit the structure, we could use meta struct 3 to get minimum dominating path for $L_{k+3,k+3}$. Then, we could use meta struct 1 to get $L_{k+3,n}$ when $n \geq k+3$ as shown in Figure~\ref{fig:path3_1} which hold the structure for $k+3$. We could use the same approach to construct when $m \equiv 2 \pmod 3$, $n \equiv 2 \pmod 3$ as shown in Figure~\ref{fig:path3_2}.

When $m \equiv 0 \pmod 3$, $n \equiv 2 \pmod 3$, we have two structures (a) and (b) as shown in Figure~\ref{fig:path3_3} and for different $n$ we also have two structures shown in ~\ref{fig:path3_2}. Assume result hold for $m=k$, and the critical point between structure (a) and structure (b) is $n=l$ when $m=k$. Specifically, when $n \leq l$, $L_{k,n}$ has structure (a) and otherwise, $L_{k,n}$ has structure (b). For $L_{k,n}$ with structure (a), we could construct from $L_{k+3, l}$. Since structure (a) could only use meta struct 3 or meta struct 2, we use meta struct 3 on $L_{k+3,l}$ until it satisfies the condition of meta struct 2. And we hold result for $m=k+1$.

Therefore, we have following results.

When $m \equiv 0 \pmod 3$, $n \equiv 0 \pmod 3$, assume $m \leq n$, we have $\gamma_l(L_{m,n})=3ab+2a-2$.
...
\begin{minipage}[c]{0.45\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/path_7.pdf}
\caption{Minimum dominating path when $m \equiv 1 \pmod 3$, $n \equiv 1 \pmod 3$.}
\label{fig:path4_1}
\end{minipage}%
\end{figure}

We could get minimum dominating path of $L_{4,4}$ in Figure~\ref{fig:path4_1}. Therefore, we could use two meta structs 2 to construct minimum dominating path of $L_{4,7}$ and we could use meta struct 2 to construct minimum dominating path of $L_{7,7}$. And we could construct minimum dominating path of $L_{4,n}$ when $n \geq 10$ from $L_{1,n}$ with a meta struct 1. Finally, for $m \geq 7$, $n \geq 10$, we could only use meta struct 3 to construct minimum dominating path of $L_{m,n}$ from $L_{m-3,n}$ which get the result in Figure~\ref{fig:path4_1}.
...
In actual scenery, 3D space might be irregular. Therefore, we give a approach to concatenate 2D trajectories to a 3D trajectory.

First, we divide 3D space into cuboids and we divide gotten 3D irregular grid into multiple layers of 2D irregular grids. Then, for each irregular grid, we could divide it into regular 2D grids in which we could use proposed algorithm to construct minimum dominating path. Finally, we could simple connect minimum dominating path for regular grids since the start points of minimum dominating path always lay in the boundary.
% XXXXXXXX
...
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/eva_compare1_2.pdf}
...
\begin{minipage}[c]{0.23\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/eva_compare2_2.pdf}
...
\begin{minipage}[c]{0.23\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/eva_compare3_2.pdf}
\caption{Battery coverage of three algorithms with different granularity of measurement time.}
\label{fig:comp3}
\end{minipage}
\begin{minipage}[c]{0.23\textwidth}
\centering
\includegraphics[width=1.0\linewidth]{pix/sensing-traj/eva_compare4_2.pdf}
...
\begin{minipage}[t]{0.3\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{pix/sensing-traj/eva_timedomain_2.pdf}
\caption{Comparison of the Adaptive Monitoring Algorithm, Greedy Algorithm and Sequential Selection.}
...
\begin{minipage}[t]{0.3\textwidth}
\centering
\includegraphics[width=0.8\linewidth]{pix/sensing-traj/eva_battery_2.pdf}
\caption{Comparison of the Adaptive Monitoring Algorithm, Greedy Algorithm and Sequential Selection.}
\label{fig:battery}
\end{minipage}
\end{figure*}
...
\begin{enumerate}
  \item \emph{Row-first}: We select the minimum dominating set in grid first and select an OL at boundary as start point. Then, we use row-first form a suboptimal trajectory by prior choosing OLs row by row (the longer side of the grid).
  \item \emph{Objective Greedy}: We select the minimum dominating set in grid first and select a point at boundary as start point. Then, we follow the greedy algorithm that
      \[
